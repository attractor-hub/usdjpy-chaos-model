...
new Chart(document.getElementById('c2'),{{
  ...
  options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}}}},
    scales:{{
      ...
      y:{{grid:{{color:'rgba(42,48,69,.5)'}},ticks:{{color:'#4a9eff',callback:v=>'¥'+v.toFixed(1)}},position:'left'}},
...
// model compare
const modelVals=[D.chaos_preds[0].mean,D.ar_preds[0],D.mom_preds[0],D.ens[0].mean];
const modelMin=Math.min(...modelVals);
const modelMax=Math.max(...modelVals);
const modelPad=Math.max((modelMax-modelMin)*0.35,0.05);
const modelYMin=modelMin-modelPad;
const modelYMax=modelMax+modelPad;
new Chart(document.getElementById('c3'),{{
  type:'bar',data:{{
    labels:['カオス k-NN','AR(5)','モメンタム','アンサンブル'],
    datasets:[{{data:modelVals,
  ...
  options:{{responsive:true,maintainAspectRatio:false,
    ...
    scales:{{x:{{grid:{{color:'rgba(42,48,69,.8)'}},ticks:{{color:'#6a7090'}}}},y:{{grid:{{color:'rgba(42,48,69,.8)'}},ticks:{{color:'#6a7090',callback:v=>'¥'+v.toFixed(2)}},min:modelYMin,max:modelYMax}}}}
  }}
}});
</script>
</body>
