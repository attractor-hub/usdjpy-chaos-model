...
# --- k-NN方向コンセンサスフィルター ---
# K_NN近傍のうち予測方向に一致した割合が閾値未満の日は取引スキップ
USE_KNN_CONSENSUS    = False  # True: フィルター有効 / False: 無効(非推奨: 優良トレードを除外する傾向)
KNN_CONSENSUS_THRESH = 0.60   # 60%以上の近傍が同方向なら取引
...
DA_MONITOR_DAYS   = 10     # 何日連続で閾値以下なら発動(ノイズ除外)
...
def fetch_japan10y(dates):
    ...
    日本10年国債利回り。
    取得優先順位:
      1. stooq.com (10yjp.b) — 無料・APIキー不要・日次
      2. Yahoo Finance ^JGB  — delisted済みのためほぼ失敗
      3. 定数 1.0% フォールバック
    """
    import urllib.request, ssl, io, csv
...
def chaos_forecast(Xe, prices, regimes, offset, idx, horizons=HORIZONS,
                   k=K_NN, theiler=None, return_consensus=False):
    """
    位相空間k-NN。時点idxまでの情報のみ使用。
    theiler: 時間的に近接した(窓が重複する)ベクトルの除外幅
    予測は近傍の「相対変化率」を現在価格に適用する
    (絶対水準の平均だと価格水準が異なる過去近傍で破綻するため)
    return_consensus=True: (予測値リスト, h=1コンセンサス比率) を返す
    """
    t_e = idx - offset
    if t_e < k + 50:
        return ([float(prices[idx])] * horizons, None) if return_consensus else [float(prices[idx])] * horizons
    if theiler is None:
    ...
    for h in range(1, horizons + 1):
        ...
        ws = 1.0 / (adj[sel] + 1e-8)
        ws /= ws.sum()
        pred = float(prices[idx] * np.dot(rel, ws))
        out.append(pred)
        # h=1のコンセンサス比率: 近傍のうち予測方向と同じ向きの割合
        if h == 1 and return_consensus and len(sel) > 0:
            pred_dir = 1 if pred > prices[idx] else -1
            neighbor_dirs = np.where(rel > 1.0, 1, -1)
            consensus_ratio = float(np.mean(neighbor_dirs == pred_dir))
    if return_consensus:
        return out, consensus_ratio
    return out

def ar_forecast(prices, idx, p=AR_ORDER, steps=HORIZONS, lookback=300):
...
def pnl_backtest(prices, dates_dt, ens_pred, eval_idxs, r_usd, lag=0,
                 macro_signal=None, hurst_arr=None, up_prob_map=None, knn_consensus_map=None):
    """
    ...
    for idx in eval_idxs:
        ...
        if USE_CONSEC_SKIP and pos_raw != 0 and consec_count >= CONSEC_SKIP_N:
            pos_raw = 0
        # k-NNコンセンサスフィルター: 近傍の方向一致率が低い日はスキップ
        if USE_KNN_CONSENSUS and knn_consensus_map is not None and pos_raw != 0:
            cr = knn_consensus_map.get(s, float("nan"))
            if not np.isnan(cr) and cr < KNN_CONSENSUS_THRESH:
                pos_raw = 0
...
.da-badge{display:inline-flex;align-items:center;gap:6px;padding:4px 12px;border-radius:2px;font-size:11px;letter-spacing:1px;text-transform:uppercase;font-weight:500;border:1px solid;margin-top:6px;margin-left:6px}
.da-badge.hidden{display:none}
.da-badge.da-warn{background:rgba(255,200,0,.12);color:#ffc800;border-color:rgba(255,200,0,.35)}
.da-badge.da-stop{background:rgba(255,60,60,.15);color:#ff4444;border-color:rgba(255,60,60,.40)}
...
<div class="hdr">
  ...
  <div class="hdr-r">
    ...
    <div style="display:flex;align-items:center;flex-wrap:wrap;gap:0">
      ...
      <div id="da-badge" class="da-badge hidden"></div>
    </div>
  </div>
</div>
...
<div class="pn" id="p1">
  ...
  <div class="card">
    <div class="card-t">Rolling Direction Accuracy &mdash; <span id="da-window"></span>d Window (Model Health Monitor)</div>
    <div class="cw h200"><canvas id="c1da"></canvas></div>
    <div class="note" id="da-note"></div>
  </div>
</div>
...
// DA監視バッジ
const DA=D.rolling_da;
const daBadge=document.getElementById('da-badge');
if(DA&&DA.use_monitor&&DA.status!=='ok'){
  daBadge.className='da-badge '+(DA.status==='stop'?'da-stop':'da-warn');
  const pct=DA.current!=null?(DA.current*100).toFixed(1)+'%':'—';
  const days=DA.status==='stop'?DA.consec_stop:DA.consec_warn;
  const icon=DA.status==='stop'?'⛔':'⚠';
  daBadge.textContent=`${icon} DA ${DA.status.toUpperCase()} ${pct} ${days}日連続`;
  daBadge.classList.remove('hidden');
}else{
  daBadge.classList.add('hidden');
}
...
{
  const DA=D.rolling_da;
  document.getElementById('da-window').textContent=DA.window;
  const daNote=`監視ルール: ${DA.warn*100}%以下が${DA.days}日連続→WARNING ／ ${DA.stop*100}%以下が${DA.days}日連続→STOP推奨。`
    +` 現在: ${DA.current!=null?(DA.current*100).toFixed(1)+'%':' —'} (${DA.status.toUpperCase()})`;
  document.getElementById('da-note').textContent=daNote;
  new Chart(document.getElementById('c1da'),{
    type:'line',
    data:{labels:DA.dates,datasets:[
      {label:'Rolling DA',data:DA.values,borderColor:'#00d4ff',borderWidth:1.5,
       pointRadius:0,fill:false,tension:.3,spanGaps:true},
    ]},
    options:{responsive:true,maintainAspectRatio:false,
      plugins:{legend:{display:false},
        annotation:{annotations:{
          warn:{type:'line',yMin:DA.warn,yMax:DA.warn,borderColor:'rgba(255,200,0,.6)',borderWidth:1,borderDash:[4,3],
            label:{content:'WARNING '+DA.warn*100+'%',display:true,position:'end',color:'#ffc800',font:{size:9}}},
          stop:{type:'line',yMin:DA.stop,yMax:DA.stop,borderColor:'rgba(255,60,60,.6)',borderWidth:1,borderDash:[4,3],
            label:{content:'STOP '+DA.stop*100+'%',display:true,position:'end',color:'#ff4444',font:{size:9}}},
          mid:{type:'line',yMin:0.5,yMax:0.5,borderColor:'rgba(0,255,136,.25)',borderWidth:1,borderDash:[2,4]},
        }}
      },
      scales:{
        x:{grid:{color:'rgba(30,45,69,.4)'},ticks:{color:'#556688',maxTicksLimit:10,maxRotation:0,
          callback:(v,i,tks)=>{const l=DA.dates[i];return l?l.slice(5):l;}}},
        y:{grid:{color:'rgba(30,45,69,.5)'},ticks:{color:'#8899bb',callback:v=>(v*100).toFixed(0)+'%'},
          suggestedMin:0.35,suggestedMax:0.65}
      }
    }
  });
}
...
def main():
    ...
    # ローリング方向一致率(DA_MONITOR_WINDOW日窓)
    rolling_da_dates, rolling_da_vals = [], []
    for j, idx in enumerate(eval_idxs):
        if idx + 1 >= N or np.isnan(ens_pred[0, idx]):
            continue
        win = [eval_idxs[k] for k in range(max(0, j - DA_MONITOR_WINDOW), j)
               if eval_idxs[k] + 1 < N and not np.isnan(ens_pred[0, eval_idxs[k]])]
        if len(win) < 20:
            rolling_da_dates.append(dates[idx])
            rolling_da_vals.append(None)
            continue
        da_val = float(np.mean([
            1 if (ens_pred[0, i] - prices[i]) * (prices[i+1] - prices[i]) > 0 else 0
            for i in win
        ]))
        rolling_da_dates.append(dates[idx])
        rolling_da_vals.append(round(da_val, 4))
    # 直近の連続下回り日数を末尾から数える
    _valid_da = [v for v in rolling_da_vals if v is not None]
    _current_da = _valid_da[-1] if _valid_da else None
    _consec_warn = _consec_stop = 0
    for _v in reversed(_valid_da):
        if _v <= DA_MONITOR_WARN:
            _consec_warn += 1
        else:
            break
    for _v in reversed(_valid_da):
        if _v <= DA_MONITOR_STOP:
            _consec_stop += 1
        else:
            break
    _da_status = ("stop" if _consec_stop >= DA_MONITOR_DAYS
                  else ("warn" if _consec_warn >= DA_MONITOR_DAYS else "ok"))
    print(f"Rolling DA (直近{DA_MONITOR_WINDOW}日): {_current_da*100:.1f}% | "
          f"ステータス: {_da_status} (warn連続{_consec_warn}日 / stop連続{_consec_stop}日)")
    ...
    # k-NNコンセンサスマップ構築(eval_idxs全日)
    knn_consensus_map = {}
    if USE_KNN_CONSENSUS:
        print("[INFO] k-NNコンセンサスマップ構築中...")
        for _idx in eval_idxs:
            _, _cr = chaos_forecast(Xe, prices, regimes, offset, _idx, return_consensus=True)
            if _cr is not None:
                knn_consensus_map[_idx] = _cr
    ...
    pnl_summary, pnl_recs   = pnl_backtest(prices, dates_dt, ens_pred, eval_idxs, r_usd, lag=0,
                                             ...
                                             knn_consensus_map=knn_consensus_map)
    ...
    payload = {
        ...
        "rolling_da": {
            "dates":       rolling_da_dates[-180:],
            "values":      rolling_da_vals[-180:],
            "window":      DA_MONITOR_WINDOW,
            "warn":        DA_MONITOR_WARN,
            "stop":        DA_MONITOR_STOP,
            "days":        DA_MONITOR_DAYS,
            "status":      _da_status,
            "consec_warn": _consec_warn,
            "consec_stop": _consec_stop,
            "current":     _current_da,
            "use_monitor": USE_DA_MONITOR,
        },
